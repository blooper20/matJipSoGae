...
//        cell.imageView?.image = UIImage(named: "swift.png") // cell에 image를 삽입
        let cell = tableView.dequeueReusableCell(withIdentifier: "myCell", for: indexPath) as! MyTableViewCell // cell의 아이디가 "myCell"인 것을 사용
        // as를 사용해서 부모를 자식으로 바꾼다. =DownCasting
        cell.myLable.text = indexPath.description // myLable의 text를 indexPath.description로 초기화
